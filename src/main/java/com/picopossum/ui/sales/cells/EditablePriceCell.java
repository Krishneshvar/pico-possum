...
public class EditablePriceCell extends TableCell<CartItem, CartItem> {
    ...
    @Override public void cancelEdit() { 
        super.cancelEdit(); 
        if (getItem() != null) setText(com.picopossum.shared.util.CurrencyUtil.format(getItem().getPricePerUnit())); 
        setGraphic(null); 
    }
    ...
    @Override public void updateItem(CartItem it, boolean e) { 
        ...
        else { setText(com.picopossum.shared.util.CurrencyUtil.format(it.getPricePerUnit())); setGraphic(null); } 
    }
    ...
    public void commitEdit(CartItem it) { 
        if (tf != null && it != null) { 
            try { 
                BigDecimal v = new BigDecimal(tf.getText().replace(com.picopossum.shared.util.CurrencyUtil.getSymbol(), "").replace(",", "").trim()).max(BigDecimal.ZERO); 
                BigDecimal m = it.getProduct().mrp(); 
                if (v.compareTo(m) > 0) { 
                    NotificationService.warning("Price cannot exceed MRP (" + com.picopossum.shared.util.CurrencyUtil.format(m) + ")"); 
                    v = m; 
                } 
