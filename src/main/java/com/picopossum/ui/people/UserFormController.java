...
public class UserFormController extends AbstractFormController<User> {
    ...
    protected void setupValidators() {
        if (nameField != null) {
            com.picopossum.ui.common.validation.FieldValidator.of(nameField)
                .addValidator(com.picopossum.ui.common.validation.Validators.required("Full name"))
                .validateOnType();
        }

        if (usernameField != null) {
            com.picopossum.ui.common.validation.FieldValidator.of(usernameField)
                .addValidator(com.picopossum.ui.common.validation.Validators.required("Username"))
                .addValidator(com.picopossum.ui.common.validation.Validators.noSpaces("Username"))
                .validateOnType();
        }

        if (statusCombo != null) {
             com.picopossum.ui.common.validation.FieldValidator.of(statusCombo)
                .addValidator(com.picopossum.ui.common.validation.Validators.notNull("Status"))
                .validateOnFocusLost(); // Combo updates are better on focus lost or selection change
        }

        if (passwordField != null) {
             com.picopossum.ui.common.validation.FieldValidator.of(passwordField)
                .addValidator(val -> {
                    if (isCreateMode() && (val == null || val.isEmpty())) 
                        return com.picopossum.ui.common.validation.ValidationResult.error("Password is required");
                    if (val != null && !val.isEmpty() && val.length() < 6) 
                        return com.picopossum.ui.common.validation.ValidationResult.error("Min 6 characters required");
                    return com.picopossum.ui.common.validation.ValidationResult.success();
                })
                .validateOnType();
        }
    }
